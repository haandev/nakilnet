const CustomViewCtor = (options) => {
  return function CustomView(provide, defineClass) {
    ...
    function CustomView(multiRouteModel) {
      ...
      this.outputElement = window.$("<div></div>").appendTo("#viewContainer")
    ...
    defineClass(CustomView, {
      ...
      processRoute: function (index, route) {
        ...
        var processorName =
          CustomView.routeProcessors[route.properties.get("type")]
      ...
      createMasstransitRouteOutput: function (route) {
        ...
        for (var i = 0, l = route.getPaths().length; i < l; i++) {
          ...
          for (var j = 0, k = path.getSegments().length; j < k; j++) {
            result.push(
              "<li>" + path.getSegments()[j].properties.get("text") + "</li>"
            )
          }
        }
        return result.join("")
